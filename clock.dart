// Copyright (c) 2012, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.

library clock;

import 'dart:html';
import 'dart:math';

import 'model.dart';
import 'package:web_ui/web_ui.dart';

part 'balls.dart';
part 'numbers.dart';

void addTodo(Event e) {
  e.preventDefault(); // don't submit the form
  var input = query('#new-todo');
  if (input.value == '') return;
  app.todos.add(new Todo(input.value));
  input.value = '';
}

InputElement add_alarm;
DateTime alarm_added;
UListElement my_alarm;
Element setAlarm;
DateTime alarmTime;

InputElement alarm_play;
UListElement Playing;
Element setSong;
//String useThisSong;


void main() {
  add_alarm = query("#add-alarm");
  my_alarm = query("#my-alarms");
  setAlarm = query("#submit");
//  alarm_added = add_alarm;
  setAlarm.onClick.listen(makeAlarm);  
//  add_alarm.onChange.listen(showTheAlarm);
  
  alarm_play = query("#alarm-play");
  Playing = query("#playing");
  alarm_play.onChange.listen(playingThis);
  setSong = query("#setSong");
 
  
  new CountDownClock();
}



//making an alarm to set off

void makeAlarm (Event poops){
  int millis = add_alarm.valueAsNumber;
  if( millis.isNaN ) 
  {
     return;
  }
  
  alarmTime = new DateTime.fromMillisecondsSinceEpoch(millis);
  int hour, minute;
  
  minute = alarmTime.minute;
  hour = (alarmTime.hour + 5)%24;
}

//attempting to show the alarm that has been set

//void showTheAlarm (Event happens){
//  var mySetAlarm = new LIElement();
//  mySetAlarm.text = alarm_added;
//  alarm_added.children.add(mySetAlarm);
//}

void playingThis (Event e){
  var newSong = new LIElement();
  newSong.text = alarm_play.value;
  alarm_play.value='';
  Playing.children.add(newSong);
  
  //useThisSong = alarm_play.value;
}

double fpsAverage;

/**
 * Display the animation's FPS in a div.
 */
void showFps(num fps) {
  if (fpsAverage == null) {
    fpsAverage = fps;
  } else {
    fpsAverage = fps * 0.05 + fpsAverage * 0.95;

    //query("#notes").text = "${fpsAverage.round().toInt()} fps";
  }
}

//clock

class CountDownClock {
  static const int NUMBER_SPACING = 19;
  static const double BALL_WIDTH = 19.0;
  static const double BALL_HEIGHT = 19.0;

  List<ClockNumber> hours = new List<ClockNumber>.fixedLength(2);
  List<ClockNumber> minutes = new List<ClockNumber>.fixedLength(2);
  List<ClockNumber> seconds = new List<ClockNumber>.fixedLength(2);
  int displayedHour = -1;
  int displayedMinute = -1;
  int displayedSecond = -1;
  Balls balls = new Balls();

  CountDownClock() {
    var parent = query("#canvas-content");

    createNumbers(parent, parent.clientWidth, parent.clientHeight);

    updateTime(new DateTime.now());

    window.requestAnimationFrame(tick);
  }

  void tick(num time) {
    updateTime(new DateTime.now());
    balls.tick(time);
    window.requestAnimationFrame(tick);
  }

  void updateTime(DateTime now) {
    if (now.hour != displayedHour) {
      setDigits(pad2(now.hour), hours);
      displayedHour = now.hour;
    }

    if (now.minute != displayedMinute) {
      setDigits(pad2(now.minute), minutes);
      displayedMinute = now.minute;
    }

    if (now.second != displayedSecond) {
      setDigits(pad2(now.second), seconds);
      displayedSecond = now.second;
    }
    
    if(alarmTime == null)
    {
      return;
    }
    
    if(now.hour == (alarmTime.hour+5)%24 && now.minute == alarmTime.minute )
    {
      window.open( "${alarm_play.value}" );
     // window.open("http://www.breadfish.de","http://www.breadfish.de");
      alarmTime = null;
    }
   
    
  }

  void setDigits(String digits, List<ClockNumber> numbers) {
    for (int i = 0; i < numbers.length; ++i) {
      int digit = digits.codeUnitAt(i) - '0'.codeUnitAt(0);
      numbers[i].setPixels(ClockNumbers.PIXELS[digit]);
    }
  }

  String pad3(int number) {
    if (number < 10) {
      return "00${number}";
    }
    if (number < 100) {
      return "0${number}";
    }
    return "${number}";
  }

  String pad2(int number) {
    if (number < 10) {
      return "0${number}";
    }
    return "${number}";
  }

  
  //creating the numbers
  
  void createNumbers(Element parent, num width, num height) {
    DivElement root = new DivElement();
    makeRelative(root);
    root.style.textAlign = 'center';
    query("#canvas-content").nodes.add(root);

    double hSize = (BALL_WIDTH * ClockNumber.WIDTH + NUMBER_SPACING) * 6
        + (BALL_WIDTH + NUMBER_SPACING) * 2;
    hSize -= NUMBER_SPACING;

    double vSize = BALL_HEIGHT * ClockNumber.HEIGHT;

    double x = (width - hSize) / 2;
    double y = (height - vSize) / 3;

    for (int i = 0; i < hours.length; ++i) {
      hours[i] = new ClockNumber(this, x, Balls.BLUE_BALL_INDEX);
      root.nodes.add(hours[i].root);
      setElementPosition(hours[i].root, x, y);
      x += BALL_WIDTH * ClockNumber.WIDTH + NUMBER_SPACING;
    }

    root.nodes.add(new Colon(x, y).root);
    x += BALL_WIDTH + NUMBER_SPACING;

    for (int i = 0; i < minutes.length; ++i) {
      minutes[i] = new ClockNumber(this, x, Balls.RED_BALL_INDEX);
      root.nodes.add(minutes[i].root);
      setElementPosition(minutes[i].root, x, y);
      x += BALL_WIDTH * ClockNumber.WIDTH + NUMBER_SPACING;
    }

    root.nodes.add(new Colon(x, y).root);
    x += BALL_WIDTH + NUMBER_SPACING;

    for (int i = 0; i < seconds.length; ++i) {
      seconds[i] = new ClockNumber(this, x, Balls.GREEN_BALL_INDEX);
      root.nodes.add(seconds[i].root);
      setElementPosition(seconds[i].root, x, y);
      x += BALL_WIDTH * ClockNumber.WIDTH + NUMBER_SPACING;
    }
  }
}

void makeAbsolute(Element elem) {
  elem.style.left = '0px';
  elem.style.top = '0px';
  elem.style.position = 'absolute';

}

void makeRelative(Element elem) {
  elem.style.position = 'relative';
}

void setElementPosition(Element elem, double x, double y) {
  elem.style.transform = 'translate(${x}px, ${y}px)';
}

void setElementSize(Element elem, double l, double t, double r, double b) {
  setElementPosition(elem, l, t);
  elem.style.right = "${r}px";
  elem.style.bottom = "${b}px";
}
